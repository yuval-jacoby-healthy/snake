...
class Agent(object):
    ...
    def __init__(self, id, policy, policy_args, board_size, logq, game_duration, score_scope, name=''):
        """
...
class Game(object):
    ...
    def __init__(self, args):
        ...
        for i, (policy, pargs) in enumerate(self.policies):
            ...
            if len(self.name) > i:
                name = self.name[i]
            self.players.append(Agent(i, policy, pargs, self.board_size, self.logq, self.game_duration, self.score_scope, name=name))
    ...
    def render(self, r):
        ...
        for i in range(len(self.scores)):
            ...
            if hasattr(self, "players"):
                print(f"Player {self.players[i].name}: {np.mean(self.scores[i][-scope:]) : .4f}")
            else:
                print(f"Player {i}: {np.mean(self.scores[i][-scope:]) : .4f}")
